...
/**
 * Returns true when a Sentry event represents an HTTP 404 response.
 *
 * Matches on:
 *   - tags['http.status_code'] === '404' / 404
 *   - contexts.response.status_code === 404
 *   - transaction / message / title containing the trailing `- 404`
 *     suffix used by the Express response instrumentation
 *     (e.g. `[RESPONSE] GET /phpinfo.php - 404`).
 *
 * Keep this filter conservative: only 404s, nothing else.
 */
function is404Event(event) {
  if (!event) return false;

  const tagStatus =
    event.tags &&
    (event.tags["http.status_code"] ?? event.tags.http_status_code);
  if (tagStatus !== undefined && String(tagStatus) === "404") {
    return true;
  }

  const ctxStatus = event.contexts?.response?.status_code;
  if (ctxStatus !== undefined && Number(ctxStatus) === 404) {
    return true;
  }

  const haystacks = [event.transaction, event.message, event.title].filter(
    Boolean
  );
  for (const s of haystacks) {
    if (typeof s === "string" && /\b-\s*404(\b|$)/.test(s)) {
      return true;
    }
  }
  ...
  try {
    ...
    Sentry.init({
      ...
      beforeSend(event, hint) {
        // ---------------------------------------------------------------
        // Drop HTTP 404 noise (unauthenticated scanner traffic).
        //
        // The vast majority of issues that reach this project are
        // `[RESPONSE] GET/POST /<path> - 404` events generated by bots
        // probing for phpinfo.php, .env*, /wp-*, /license.txt, etc.
        // They are not application errors and they burn the monthly
        // error budget. Filter them out at the SDK level. Extend the
        // checks below if new noise patterns appear.
        // ---------------------------------------------------------------
        if (is404Event(event)) {
          return null;
        }
      ...
      // Drop 404 performance transactions too — these show up as
      // `[RESPONSE] GET /<path> - 404` and dominate the issue feed.
