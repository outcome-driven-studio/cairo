...
class NewSyncRoutes {
    ...
    try {
      ...
      for (const campaign of allCampaigns) {
        if (campaign.platform === "lemlist") {
          ...
          logger.info(
            `[3/3] Processing campaign: ${campaign.name}, ${JSON.stringify(
              campaign,
              null,
              2
            )}`
          ...
          logger.info(
            `[3/3] Found ${leads.length} leads for campaign: ${campaign.name}`
          );
          ...
          logger.info(
            `[3/3] Found ${activities.length} activities for campaign: ${
              campaign.name
      ...
      logger.error(`❌❌❌ INITIAL SYNC ROUTE FAILED: ${error.message}`, {
        stack: error.stack,
      });
  ...
  async handleBatchSmartleadSync(req, res) {
    try {
      ...
      const campaigns = await this.smartleadService.getCampaignsByIds(
        campaignIds
      );
      ...
      for (const campaign of preProcessedCampaigns) {
        logger.info(
          `[BATCH] Processing campaign: ${campaign.name}, ${JSON.stringify(
            campaign,
            null,
            2
          )}`
      ...
      res.status(202).json({
        status: "accepted",
        message:
          "Batch smartlead sync processing started. Check logs for progress.",
      });
    } catch (error) {
      logger.error(
        `❌❌❌ BATCH SMARTLEAD SYNC ROUTE FAILED: ${error.message}`,
        { stack: error.stack }
      );
      await monitoring.captureError(error, {
        type: "batch-smartlead-sync-route",
      });
    }
  }

  async getOrCreateUserByLinkedInUrl(
    linkedinUrl,
    userData = {},
    campaign = null
  ) {
      ...
      try {
        ...
        if (existingUser) {
          ...
          if (updates.length > 0) {
            ...
            logger.info(
              `[DB] Updated existing user with supersync_id: ${supersyncId} for linkedin_url: ${linkedinUrl}`
            );
            return supersyncId;
          } else {
            const supersyncId = existingUser.id;
            logger.info(
              `[DB] Found existing user with supersync_id: ${supersyncId} for linkedin_url: ${linkedinUrl} (no updates needed)`
            );
            return supersyncId;
          }
        } else {
          ...
          const newUser = await this.upsertUserSource(
            userDataForCreation,
            campaign
          );
          if (newUser) {
            ...
            logger.info(
              `[DB] Created new user with supersync_id: ${supersyncId} for linkedin_url: ${linkedinUrl}`
            );
            return supersyncId;
          } else {
            logger.error(
              `[DB] Failed to create user for linkedin_url: ${linkedinUrl}`
            );
            return null;
          }
        }
        ...
        logger.error(
          `[DB] Error checking/creating user ${error.message}, ${error.stack}`
        );
      }
    } else {
      try {
        ...
        logger.error(
          `[DB] Error checking/creating user for linkedin_url: ${linkedinUrl}`,
          error
        );
        return null;
      }
  ...
  async processLemlistActivities(activities, leadMap) {
    ...
    for (const [leadId, leadData] of leadMap) {
      ...
      if (!linkedinUrl) {
        logger.warn(
          `[PROCESS] Skipping lead ${leadId} - no linkedin_url found`
        );
        continue;
      }
      ...
      logger.info(
        `[PROCESS] Getting or creating user for lead ${leadId} with linkedin_url: ${linkedinUrl}, ${JSON.stringify(
          userData,
          null,
          2
        )}`
      ...
      logger.info(
        `[PROCESS] Got or created user for lead ${leadId} with supersync_id: ${supersyncId}`
      );

      if (supersyncId) {
        leadToSupersyncMap.set(leadId, supersyncId);
        logger.info(
          `[PROCESS] Mapped lead ${leadId} to supersync_id: ${supersyncId} for linkedin_url: ${linkedinUrl}`
        );
      } else {
        logger.warn(
          `[PROCESS] Failed to get/create user for lead ${leadId} with linkedin_url: ${linkedinUrl}`
        );
      }
    }

    logger.info(
      `[PROCESS] Processed ${leadToSupersyncMap.size} leads with supersync IDs out of ${leadMap.size} total leads`
    );

    // Now process activities using the supersync IDs
    for (const activity of activities) {
      logger.info(
        `[3/3] Processing activity: ${activity.type}, ${JSON.stringify(
          activity,
          null,
          2
        )}`
      ...
      if (
        !activity.type ||
        !activity.type.toLowerCase().startsWith("linkedin")
      ) {
        continue;
      }
      ...
      if (activity.leadId) {
        ...
        if (mappedLead) {
          ...
          logger.info(
            `[PROCESS] Found lead data for leadId ${
              activity.leadId
            }: ${JSON.stringify(mappedLead).substring(0, 200)}...`
          );
        }
      }
      ...
      if (!leadId || !linkedinUrl) {
        logger.warn(
          `[PROCESS] Skipping Lemlist activity due to missing leadId or linkedin_url. Activity Type: ${activity.type}, LeadId: ${leadId}, LinkedIn: ${linkedinUrl}`
        );
        continue;
      }
      ...
      if (!supersyncId) {
        logger.warn(
          `[PROCESS] No supersync_id found for leadId: ${leadId}, linkedin_url: ${linkedinUrl}. Skipping activity processing.`
        );
        continue;
      }
      ...
      const improvedEventKey = eventKeyGenerator.generateLemlistKey(
        {
          ...
          lead: { email: email || `user_${supersyncId}` },
        },
        activity.campaignId,
        "playmaker"
      );
  ...
  async processSmartleadCampaign(campaign) {
    logger.info(
      `[3/3] Processing Smartlead campaign: ${campaign.name} (ID: ${campaign.id}, External ID: ${campaign.external_id})`
    );
    ...
    try {
      ...
      for (const lead of leads) {
        ...
        logger.info(
          `[PROCESS] Processing Smartlead lead ${leadData.id} with email: ${
            leadData.email
          }, linkedin_url: ${leadData.linkedin_profile}, ${JSON.stringify(
            leadData,
            null,
            2
          )}`
        ...
        if (!email) {
          logger.warn(
            `[PROCESS] Skipping Smartlead lead ${leadId} - no email found`
          );
          continue;
        }
        ...
        logger.info(
          `[PROCESS] Getting or creating user for Smartlead lead ${leadId} with email: ${email}, linkedin_url: ${
            linkedinUrl || "null"
        ...
        logger.info(
          `[PROCESS] Got or created user for Smartlead lead ${leadId} with supersync_id: ${supersyncId}`
        );

        if (supersyncId) {
          leadToSupersyncMap.set(email, supersyncId);
          logger.info(
            `[PROCESS] Mapped Smartlead lead ${leadId} to supersync_id: ${supersyncId} for email: ${email}`
          );
        } else {
          logger.warn(
            `[PROCESS] Failed to get/create user for Smartlead lead ${leadId} with email: ${email}`
          );
        }
      }

      logger.info(
        `[PROCESS] Processed ${leadToSupersyncMap.size} Smartlead leads with supersync IDs out of ${leads.length} total leads`
      );

      // Process different types of events
      const processEvents = async (eventType, eventFetcher) => {
        logger.info(
          `[PROCESS] Processing ${eventType} events for campaign: ${campaign.name}`
        );
        try {
          const eventsData = await eventFetcher(campaignId);

          logger.info(
            `[PROCESS] Got ${eventType} data for campaign: ${campaign.name}`
          );
          const events = eventsData?.data || [];
          logger.info(
            `[PROCESS] Processing ${events.length} ${eventType} events for campaign: ${campaign.name}`
          );

          for (const event of events) {
            logger.info(
              `[PROCESS] Processing ${eventType} event: ${JSON.stringify(
                event,
                null,
                2
              )}...`
            ...
            if (!event.lead_email) {
              logger.warn(
                `[PROCESS] Skipping ${eventType} event with no email: ${JSON.stringify(
                  event
                )}`
              );
              continue;
            ...
            if (!supersyncId) {
              logger.warn(
                `[PROCESS] No supersync_id found for Smartlead leadId: ${leadId}, email: ${leadEmail}. Skipping event processing.`
              );
              continue;
            }

            logger.info(
              `[PROCESS] ✅ Using supersync_id: ${supersyncId} for Smartlead ${eventType} event, leadId: ${leadId}, email: ${leadEmail}`
            );
          ...
          logger.error(
            `[PROCESS] Error processing ${eventType} events for campaign ${campaign.name}: ${error.message}`
          );
          await monitoring.captureError(error, {
      ...
      await processEvents(
        "sent",
        this.smartleadService.getSentEmails.bind(this.smartleadService)
      );
      await processEvents(
        "opened",
        this.smartleadService.getOpenedEmails.bind(this.smartleadService)
      );
      await processEvents(
        "clicked",
        this.smartleadService.getClickedEmails.bind(this.smartleadService)
      );
      await processEvents(
        "replied",
        this.smartleadService.getRepliedEmails.bind(this.smartleadService)
      );

      logger.info(
        `[PROCESS] ✅ Finished processing Smartlead campaign: ${campaign.name}`
      );
    } catch (error) {
      logger.error(
        `[PROCESS] ❌ Error processing Smartlead campaign ${campaign.name}: ${error.message}`
      );
  ...
  async insertEventSource(eventData) {
    try {
      logger.error(
        `[PROCESS] Inserting event source: ${JSON.stringify(
          eventData,
          null,
          2
        )}`
      ...
      const email =
        eventData.email || `linkedin-${eventData.user_id || "unknown"}`;
      ...
      logger.error(
        `Failed to create event for: ${eventData?.email || "unknown"}`,
        error
      );
  ...
  async upsertUserSource(userData, campaign) {
    ...
    if (!userData.email && !userData.linkedin_url) {
      logger.warn(
        "[DB] ❌ Skipping invalid user data: missing both email and linkedin_url"
      );
      return null;
    }
    ...
    try {
      ...
      if (existingUser) {
        ...
        if (updates.length > 0) {
          ...
          const updatedUser = result.rows[0];
          logger.info(
            `✅ Updated existing user with id: ${updatedUser.id} (email: ${
              updatedUser.email || "null"
            }, linkedin: ${updatedUser.linkedin_profile || "null"})`
          );
          ...
          logger.info(
            `✅ User already exists with id: ${existingUser.id}, no updates needed`
          );
          return existingUser;
        }
      } else {
        ...
        const result = await query(
          ...
          [
            ...
            JSON.stringify([
              ...(existingUser?.meta || []),
              { ...userData, platform: userData.platform },
            ]),
          ]
        );
        ...
        logger.info(
          `✅ Created new user with supersync_id: ${supersyncId} (email: ${
            email || "null"
      ...
      logger.error(
        `Failed to upsert user in user_source for: ${email || linkedinProfile}`,
        error
      );
